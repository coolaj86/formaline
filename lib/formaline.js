    ...
    if ( filelogging || recordRequest ) {
        ...
        form.on( 'startlogging', function ( req ) {
            ...
            if ( recordRequest ) {
                ...
                form.req.on( 'close', function () {
                    rlog.end();
                } );
                form.req.on( 'end', function () {
                    rlog.end();
                } );
                /**/
                if ( ok ) {
...
var formaline = function ( config ) {
    ...
    me.uploadRootDir = '/tmp/';
    me.emitProgress = false;
    me.uploadThreshold = 1024 * 1024 * 1024; // bytes
    me.maxFileSize = 1024 * 1024 * 1024; 
    me.checkContentLength = false;
    me.removeIncompleteFiles = false;
    me.holdFilesExtensions = false;
    me.serialzedFieldThreshold = 1024 * 1024 * 1024;
    me.sha1sum = false;
    me.listeners = {};
    me.logging = 'debug:off,1:on,2:on,3:off,file:off,console:on,record:off';
    me.getSessionID = null; // not undefined!! apply function doesn't work on undefined values
    me.requestTimeOut = 120000; // default for socket timeout
    me.resumeRequestOnError = true; 
    ...
    me.chunksReceived = 0;
    me.currentChunksReceived = 0; // only for checking last chunk for data progress
    me.bytesReceived = 0;
    me.endResults = null;
    me.fileStream = null;
    me.fileSize = 0;
    ...
    me.completedFiles = [];
    me.incompleteFiles = [];
    me.incompleteFilesCollection = { list: [], hash: {} };
    me.receivedFilesCollection = { list: [], hash: {} };
    me.receivedFieldsCollection = { list: [], hash: {} };
    ...
    me.res = null;
    me.sid = null;
    me.requestTimeOut = ( me.requestTimeOut <= 100 ) ? 100 : me.requestTimeOut; // normalize timeout value, minimum value is 100 millisecs
...
fproto.parse = function ( req, res, next ) {
        ...
        getProgressEmitter = function ( headerContentLength ) {
            ...
            if ( dProgress === true ) {
                return function ( isEnd ) {
                    me.emitEvent( 'progress', { bytes: me.bytesReceived, chunks: me.chunksReceived, ratio: ratio( me.bytesReceived) }, 3 );
                };
    ...
    if ( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ) {
        ...
        if ( path.existsSync( me.uploadRootDir ) ) {
            me.logger( 3, '\nformaline, upload root dir exists: "' + me.uploadRootDir + '"' );
        ...
        if ( ! path.existsSync( me.uploadRootDir ) ) { // if subdirectory doesn't already exist, create it
            ...
            me.logger( 3, '\nformaline, upload subdirectory already exists: "' + me.uploadRootDir + '"' );
        }
        ...
        if ( ctype ) {
            ...
            if ( isUpload ) {
                ...
                me.logger( 1, 'formaline, boundary : ' + me.boundBuffer + '\nformaline, boundary length: ' + me.boundBuffer.length + ' bytes' );
                me.req.addListener( 'close', me.closeConnection.bind( this ) ); // createDelegate( this, true, true ) );
...
fproto.parseMultipartData = function( chunk ){
    ...
    if ( me.choppedHeadersPrefix ){
        me.logger( 3, '\n chopped headers: '+ me.choppedHeadersPrefix );
    ...
    var writeToFileStream = function ( dataPayload, cfg ) {
            try {
                if ( dataPayload ) {
                    ...
                    me.logger( 3, '\nformaline, new data were written to this file stream  --> ', me.fileStream.path );
                    ...
                    if ( cfg && cfg.path ) {
                        ...
                        me.logger( 3, '\nformaline, a new file stream was created --> ', me.fileStream.path );
                    }else{
        ...
        if ( me.fileStream ) {
            if ( me.chopped ) { // fileStream exists, file data is chopped
                if ( resultsLength === 0 ) { // chunk is only data payload
                    me.logger( 3, '  <-- this chunk contains only data.. bytes written to disk: ' + me.bytesWrittenToDisk );
                     if ( checkSize( chunk ) ) {
                    ...
                    me.logger( 3, '<-- this chunk contains data and fields.. current bytes written to disk: ' + me.bytesWrittenToDisk + '\n' );
                    ...
                    if ( !checkSize( fileDataChunk ) ) {
                        ...
                        if ( ( fileDataChunk.length > 0 ) && ( me.bytesWrittenToDisk + fileDataChunk.length < me.uploadThreshold ) ) {
                            me.logger( 3, '<-- data part from the previous chopped file, bytes: ' + fileDataChunk.length + ', result[ 0 ] <> 0 :', results[ 0 ], '\n' );
    ...
    me.logger( 3, '\n results length --> ' + resultsLength + ', chunk #: ' + me.chunksReceived + '\n' );
    for ( var i = 0; i < resultsLength; i++ ) {
        ...
        me.logger( 3, '\n parsing headers, result #' + i, ': ', result, ', chunk #: ' + me.chunksReceived + '\n' );     
        ...
        if ( rfinish > rstart + bblength + 2 ) {
            ...
            me.logger( 3, ' seems that the end of request was reached, end payload index: ' + rfinish + ', start payload index: ' + ( rstart + bblength + 2 ) + '\n' );
        } 

        chunk.copy( hbuffer, 0, rstart, hbsize );
        hok = ( hbuffer.toString().match( crlfcrlf ) !== null );
        
        me.logger( 3, 'chunk #' + me.chunksReceived + ', cycle: ' + i + ', results[' + i + ']:', results[i], ' -> interval: ( ' + rfinish +', ' + chunk.length + ' ) \n crlfcrlf: ' + ( ( hok ) ? 'ok' : 'not found' ) );   
        me.logger( 3, 'headers -> *..* \n*' + hbuffer.toString() + '*\n' );
        ...
        if ( !hok ) { // the last result contains chopped headers
            ...
            if ( tbuff.toString().indexOf( endBoundary ) === -1 ) {
                ...
                me.logger( 3, ' <-- end of the request reached \n' );
                break;
            }
        }
        ...
        if ( fieldName ) { 
            ...
            me.logger( 3, ' fieldName parsed --> : ', fieldName[ 1 ], '\n' );
            if ( fileName ) { // file field
                ...
                me.logger( 3, ' fileName parsed --> : ', fileName[ 1 ], '\n' ); 
