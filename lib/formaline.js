...
FPROTO.emitException = function( type, isUpload, msg, logLevel, endResponse ){
...
FPROTO.emitField = function( fieldname, fieldvalue ){
        this.logger( 2, '\n formaline, field <-> name: "' + fieldname + '", value: "' + fieldvalue +'"' );
        this.emit( 'field', fieldname, fieldvalue);
};
        
FPROTO.emitFileReceived = function( sha1name, origname, path, type, size, fieldname, datasha1sum ){
...
FPROTO.emitFileRemoved = function( sha1name, origname, path, type, size, fieldname ){
...
FPROTO.emitProgress = function( bytes, chunks, ratio ){
        ...
        getProgressEmitter = ( function( headerContentLength ){
            ...
            if( dProgress === true ){
                return function( isEnd ){    
                    this.emitProgress( this.bytesReceived, this.chunksReceived, ratio( this.bytesReceived ) ); // every chunk
                };
                ...
                if( dProgress < 2 ){ dProgress = 2; }
                    return function( isEnd) {
                        if( ( ( this.chunksReceived % dProgress ) === 1 ) || isEnd ){ // mod 1 is for first chunk
                           this.emitProgress( this.bytesReceived, this.chunksReceived, ratio( this.bytesReceived ) ); //  every dProgress chunks 
                        }
                    };
            }else{
                return emptyFn;
...
FPROTO.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
        ...
        addToCompletedList = ( function( file ){
            ...
            this.emitFileReceived( path.basename( file.path ), file.origname, path.dirname( file.path ), file.ctype, this.fileSize, file.fieldname, filedatasha1sum );
        } ).bind( this ),
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                if( i < resultsLength - 1 ){
                    fileDataChunk = new Buffer( results[ i + 1 ].start - 2   - ( results[ i ].finish + 4 ) ); // + 4 ( CRLFCRLF )
                    copyBuffer( chunk, fileDataChunk, 0, results[ i ].finish + 4 , results[ i + 1 ].start - 2 ); 
                    this.emitField( fieldName[ 1 ], fileDataChunk.toString() );
                }
            }
        } // end if(fieldname)         
    } // end for
                ...
                fs.unlink( currfile, (function( err, cfile, i, len ){
                    if( err ){
                        this.emitException( 'warning', isUpload, 'warning <-> file unlink exception:' + cfile + ', directory: ' + this.uploadRootDir, 1 );
                         ...
                         this.emitFileRemoved( 2, path.basename( cfile ), ifile.origname, ifile.dir, ifile.type, ifile.rbytes, ifile.field );
                    }
                    if( i === len - 1){
                        logParserStats();
