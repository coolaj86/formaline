...
fproto.parse = function( req, res, next ){
    ...
    if( req && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' ) && 
       ( req.headers ) && ( req.headers['content-type'] ) && ( ~req.headers['content-type'].indexOf('multipart/form-data' ) || ~req.headers['content-type'].indexOf('urlencoded' ) ) ){
        ...
        this.handleXHR =  function( headers ){
        ...
        var hs = req.headers,
            ...
            //if boundary is not defined and type is multipart/form-data, it could be a custom, not standard compliant, XHR request
        ...
        if( clength ){
            try{
                ...
                if( bytes2Receive > this.uploadThreshold ){//check Max Upload Size in bytes
                    ...
                    this.logger(0,' formaline, req.headers[content-length] --> ', bytes2Receive);
                }
            }catch(error){
                //error
        ...
        if( ctype ){
            ...
            if( ~ctype.indexOf('multipart/form-data') ){
                ...
                req.addListener('data', this.parseData.bind( this ) );                
                
                req.addListener( 'end', this.sendResponse.bind( this, clength ) );
