...
fproto.emitEvent = function( type, obj, logLevel ){
    var etype = type.replace('exception','');
    this.logger( logLevel, '\n formaline, event: "' + etype + '" --> \n' , obj );
    if( type === 'end' ){
          this.emit( 'end', obj, this.res, this.next ); 
    }else if( ( type.indexOf( 'warning' ) > - 1 ) || ( type.indexOf( 'exception' ) > - 1 ) ){
        // exception event
        obj.type = etype.replace('exception','');
        this.emit( 'error', obj );
        if( obj.fatal === true ){
            if( ( this.req && this.resumeRequestOnError ) && ( ( etype !== 'aborted' ) && ( etype !== 'timeout' ) ) ){ 
                ...
                this.emit( 'end', { stats: {}, incomplete: [], files: [] }, this.res, this.next );
            }
        }
    }else{
        this.emit( etype, obj );
...
fproto.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
        ...
        addToIncompleteList = ( function( file ){
            var jsonWarnIncomplete = { isupload: true, msg:'', fatal: false };
        ...
        addToCompletedList = ( function( file ){
            ...
            //this.receivedFilesCollection[ jsonReceivedFile.hashname ] = jsonReceivedFile ;
...
fproto.sendResponseToMultipart =  function( nbytes ){ 
    ...
    if( this.removeIncompleteFiles === false ){
        ...
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            for( var i = 0, ufile = this.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[ 0 ].path + '/' + ufile[ 0 ].hashname; i < len; i++, currfile = ( ufile[i] ) ? ( ufile[ i ].path + '/' + ufile[ i ].hashname ) : null ){
