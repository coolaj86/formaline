...
var setDebuggingLevel = function( dstring, form ){
    ...
    if( filelogging || recordRequest ){
        ...
        form.on( 'startlogging', ( function( req ){   
            var fname = form.uploadRootDir + path.basename( form.uploadRootDir ).replace( '/', '' );
            if( filelogging ){
                fpath = fname + '.log';
                flog = ( filelogging ) ? ( new fs.WriteStream( fpath ) ) : ( null );
                if( dlevels[ 3 ] === 'on' ){
            ...
            if( recordRequest ){
                rpath = fname + '.req';
                rlog = ( recordRequest ) ? ( new fs.WriteStream( rpath ) ) : ( null );
