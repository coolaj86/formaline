...
fproto.closeConnection = function( cerr, isUpload ){
    var jsonConnectionErr = { isupload: isUpload, msg: '', fatal: true },
        emsg = 'connection event: ' + '"' + cerr.code + '" : ' + cerr.message + ( ( cerr.code === 'timeout' ) ? ( ', max millisecs : ' + this.requestTimeOut ) : '' );
    ...
    if( ( cerr.code === 'aborted' ) || ( cerr.code === 'timeout' ) ){
            this.sendResponseToMultipart( ( this.req.headers ) ? this.req.headers[ 'content-length' ] : 0 ); //TODO double call if error
...
fproto.sendResponseToMultipart = function ( nbytes ) {
    ...
    var logParserStats = function () { // ( function(){
            me.logger( 1, '\n (°)--/PARSER_STATS/ ' );
            me.logger( 1, '  |                          ' );
        ...
        logOverallResults = function ( updateEndTime ) { // ( function( updateEndTime ){
        ...
        resetAttributes = function () { // ( function(){
            me.chunksReceived = 0;
            me.bytesReceived = 0;
            me.bytesWrittenToDisk = 0;
            me.fileStream = null;
        ...
        groupResultsByFieldName = function ( hash ) { // ( function( hash ){
            var arr = [];
            for ( var h in hash ){
        ...
        removeFile = function ( file, last ) { //TODO if request is aborted at the end don't emit loadend?
    ...
    if ( me.removeIncompleteFiles === false ) {
        ...
        if ( me.incompleteFilesCollection.list.length === 0 ) {
          ...
          for( var i = 0, last = false, ufile = me.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[ 0 ].value.path; i < len; i++, currfile = ( ufile[i] ) ? ( ufile[ i ].value.path  ) : null, last = ( i === len - 1 ) ){
                removeFile( currfile, last );
            } //end for
        }
    }
}; // end sendResponse     
