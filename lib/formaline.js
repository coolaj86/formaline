...
var formaline = function ( config ){
    ...
    this.maxFieldSize = 1024 * 1024 * 1024; // bytes TODO
    ...
    this.maxSizeExceeded = false; // TODO
    this.startTime = 0;
    this.endTime = 0;
    this.req = null;
    this.res = null;
...
fproto.parseUrlEncodedData = function( chunk ){ // TODO add maxFieldSize checking
...
fproto.sendResponseToUrlEncoded = function(){ // TODO add stats for fields
    ...
    this.emitEvent( 'loadend', { stats: {}, incomplete: [], files: [], fields: this.receivedFieldsCollection.list }, 2 ); 
};
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
        ...
        addToIncompleteList = ( function( file ){
            ...
            if( this.incompleteFiles.indexOf( file.path ) < 0 ){
                ...
                jsonWarnIncomplete.msg = 'upload threshold was exceeded, file incomplete: ' + path.basename( file.path ) ;
        ...
        addToCompletedList = ( function( file ){
            ...
            this.receivedFilesCollection.list.push( jsonReceivedFile ); // TODO
        ...
        checkSize = ( function( buffer ){
            if( ( this.maxSizeExceeded ) || ( this.maxFieldSize < this.fileSize + buffer.length ) ){
            ...
            if( this.chopped ){ // fileStream exists, file data is chopped
                if( resultsLength === 0 ){ // chunk is only data payload
                    ...
                    if( !checkSize( fileDataChunk ) ){
                        ...
                        if( ( fileDataChunk.length > 0 ) && ( this.bytesWrittenToDisk + fileDataChunk.length < this.uploadThreshold ) ){ //TODO HERE
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                if( ( this.completedFiles.indexOf( filepath ) > -1 ) || ( this.incompleteFiles.indexOf( filepath ) > -1 ) ){ //TODO check here !!
                    fileseed = ( new Date().getTime() );
            console.log( '\n  *PATH EXISTS BEFORE-->\n', path.basename( filepath ), escapedFilename, fieldName[ 1 ], fileseed, '***' );
                    filepath = this.uploadRootDir + generateHashFileName( fileseed + '_' + escapedFilename );
            console.log( '\n  *PATH EXISTS AFTER-->', path.basename( filepath ), escapedFilename, fieldName[ 1 ], fileseed, '***' );
            console.log( 'COMPLETED:', this.completedFiles, '\n***' );
            console.log( 'INCOMPLETE:', this.incompleteFiles, '\n***\n' );
            ...
            console.log( '\n *PATH DOESN\'T EXISTS', path.basename( filepath ), escapedFilename, fieldName[ 1 ],  '***' );
            console.log( 'COMPLETED:', this.completedFiles, '\n***' );
            console.log( 'INCOMPLETE:', this.incompleteFiles, '\n***\n' );            
                }
                ...
                wok = writeToFileStream( null, {
                    ...
                    mtime: '',
                    seed: fileseed // TODO return escaped filename as name, add seed to return values
                });
                if ( !wok ){ return; }
                ...
                if( i < resultsLength - 1 ){
                    ...
                    this.receivedFieldsCollection.list.push( jsonFieldReceived ); // TODO
...
fproto.sendResponseToMultipart =  function( nbytes ){ 
    ...
    this.endResults = {
        ...
        partialFiles: this.incompleteFilesCollection.list.length
    };
