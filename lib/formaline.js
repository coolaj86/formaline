...
fproto.sendResponseToMultipart =  function( nbytes ){ 
    ...
    if( this.removeIncompleteFiles === false ){
        ...
        sendResponse( { files: groupResultsByFieldName( this.receivedFilesCollection.hash ), incomplete: this.incompleteFilesCollection.list, fields: this.receivedFieldsCollection.list, stats: this.endResults, } );
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            sendResponse( { stats: this.endResults, incomplete: [], files: groupResultsByFieldName( this.receivedFilesCollection.hash ), fields: this.receivedFieldsCollection.list } );
        }else{
          for( var i = 0, ufile = this.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[ 0 ].value.path; i < len; i++, currfile = ( ufile[i] ) ? ( ufile[ i ].value.path  ) : null ){
                fs.unlink( currfile, ( function( err, cfile, i, len ){
                    ...
                    if( i === len - 1){
                        ...
                        sendResponse( { stats: this.endResults, incomplete: this.incompleteFilesCollection.list, files: groupResultsByFieldName( this.receivedFilesCollection.hash ), fields: this.receivedFieldsCollection.list } );
                    }
