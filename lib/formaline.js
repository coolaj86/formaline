...
fproto.parse = function( req, res, next ){
    ...
    if( req && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' ) && 
        ...
        try{ //check upload dir Existence
            ...
            //fs.mkdirSync(this.uploadRootDir+'completed/','0755');
        }catch(oerr){
            ...
            try{
                ...
                this.emit('filepathexception', this.uploadRootDir, ierr.message, this.res, this.next);
                return;
            }
        }
            ...
            if( ~ctype.indexOf('multipart/form-data') ){
                    ...
                    if( this.bytesReceived <= this.uploadThreshold ){// is size allowed? 
                        if( this.chunked && this.fileStream ){// file data is chunked? && fileStream exists?
                            if( resultsLength === 0 ){//chunk is only data payload
                                ...
                                if( ( this.bytesWritedToDisk + fileData.length < this.uploadThreshold ) ){
                                    try{
                                ...
                                this.emit('filereceived',path.basename(this.fileStream.path),path.dirname(this.fileStream.path),this.fileStream.ctype,this.fileSize,this.fileStream.fieldname);
                    ...
                    for( var i = 0; i < resultsLength; i++ ){
                        ...
                        if(fieldName){
                            ...
                            if(fileName){//file field
                                ...
                                var filepath = this.uploadRootDir + fileName[1];
                                if( this.completedFiles.indexOf(filepath) < 0 ){ //file with the same name already exists
                                    ...
                                    filepath = this.uploadRootDir +(new Date().getTime())+'_'+fileName[1];
                                ...
                                if( i === resultsLength - 1 ) {//last result
                                    ...
                                    if(this.fileStream){
                                        ...
                                        if( ( this.fileSize > 0 ) && (this.incompleteFilesCollection.list.indexOf(this.fileStream.path) < 0 ) ){
                                            this.completedFiles.push(this.fileStream.path);
                                            this.emit('filereceived',path.basename(this.fileStream.path),path.dirname(this.fileStream.path),contentType[1],this.fileSize,this.fileStream.fieldname);
                                        }
                                        this.fileStream.end();
                                    }
                ...
                req.addListener('end',(function(nbytes){  
                    ...
                    if(this.removeIncompleteFiles === false){
                        this.emit('end', this.incompleteFilesCollection.list, this.res, this.next);
                    }else{
                            ...
                            for(var i = 0, ufile = this.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[0]; i < len; i++, currfile = ufile[i] ){
                                fs.unlink(currfile,(function(err,cfile,i,len){
                                    if(err){
                                        ...
                                        this.emit('fileremoved',path.basename(cfile),ifile.dir,ifile.type,ifile.rbytes,ifile.field);
                                    }
                                    if( i === len - 1){
                                        this.incompleteFilesCollection = { list: [] };
