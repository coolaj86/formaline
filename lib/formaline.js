...
fproto._emitException = function( type, isUpload, msg, logLevel, endResponse ){
        ...
        this.emit( 'exception', type, isUpload, 'formaline, ' + msg, ( endResponse === true ) );
        if( endResponse === true ) {
...
fproto.emitEvent = function( etype, obj, logLevel ){
    this.logger( logLevel, '\n formaline, event: "' + etype + '" <=> ' , obj );   
    this.emit( etype, obj );
};

fproto.emitException = function( etype, obj, logLevel ){
        this.logger( logLevel, '\n formaline, event: "'+ etype + '" <=> ' , obj );
        obj.type = etype;
...
fproto.parse = function( req, res, next ){
    ...
    var hs = req.headers,
        ...
        validatePostSize = ( function( expected, isUpload ){
            ...
            if( expected > this.uploadThreshold ){ // check Max Upload Size in bytes
                ...
                //this.emitException( 'warning', isUpload, 'warning <-> invalid content-length header, bytes to receive: ' + expected  + ', bytes allowed: ' + this.uploadThreshold, 1 );
                this.emitException( 'warning', { 
                    isupload: isUpload,
                    msg: 'invalid content-length header, bytes to receive: ' + expected  + ', bytes allowed: ' + this.uploadThreshold 
                }, 1 );
            }
            return true;
        } ).bind( this ),
            ...
            try{
                if( typeof this.getSessionID === 'function' ){
                    ...
                    if( typeof sessionID !== 'string' ){
                        //this.emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier, function \'this.getSessionID( req )\' doesn\'t return a String!', 1 );  
                        this.emitException( 'warning', { 
                            isupload: isUpload,
                            msg: 'unable to retrieve session identifier, function this.getSessionID( req ) does not return a String!'
                        }, 1 );      
                    }else{
                    ...
                    //this.emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier, configuration parameter \'this.getSessionID\' must be a function!', 1 );
                    this.emitException( 'warning', {
                        isupload: isUpload,
                        msg: 'unable to retrieve session identifier, configuration parameter this.getSessionID must be a function!'
                    }, 1 );  
                }  
              }catch( serr ){
                  //this.emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier: ' + serr.stack, 1 );
                  this.emitException( 'warning', {
                      isupload: isUpload,
                      msg: 'unable to retrieve session identifier: ' + serr.stack
                  }, 1 );            
                  return null;
              }
    ...
    if( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ){
        ...
        try{ 
            ...
            this.emitException( 'warning', {
                isupload: isUpload,
                msg: 'default directory creation exception: ' + this.uploadRootDir + ', ' + dirErr.message
            }, 1 );
            try{
                fs.mkdirSync( this.uploadRootDir, '0750' ); 
            }catch( ierr ){
                //this.emitException( 'pathexception', isUpload, 'directory creation exception: '+ this.uploadRootDir + ', ' + ierr.message, 0, true );
                this.emitException( 'pathexception', {
                    isupload: isUpload,
                    msg: 'directory creation exception: '+ this.uploadRootDir + ', ' + ierr.message,
                    fatal: true
                }, 0 );
                return;
            }
        ...
        if( clength ){ 
            try{
               ...
               this.emitException( 'headersexception', {
                    isupload: isUpload,
                    msg: 'req.headers[ content-length ]: '+ parseIntError + ', length value:' + clength,
                    fatal: true
                }, 0 );
                return;
            }
            if( ! validatePostSize( bytes2Receive, isUpload ) ){
                ...
                this.emitException( 'headersexception', {
                    isupload: isUpload,
                    msg: 'req.headers[ content-length ] exceeds max allowable: ' + bytes2Receive + ' > ' + this.uploadThreshold,
                    fatal: true
                }, 0 );
                return;
            }
            ...
            this.emitException( 'headersexception', {
                isupload: isUpload,
                msg: 'req.headers[ content-length ] not found: Parse Length Error',
                fatal: true
            }, 0 );
            return;
        }
        if( ctype ){   
            ...
            if( isUpload ){
                ...
                this.emitException( 'headersexception', {
                    isupload: isUpload,
                    msg: 'req.headers[ content-type ] --> ' + ctype + ' handler for this kind of request is not defined',
                    fatal: true
                }, 0 );
                return;
            }
            ...
            this.emitException( 'headersexception', {
                isupload: isUpload,
                msg: 'req.headers[ content-type ] not found: Parse Type Error',
                fatal: true
            }, 0 );
            return;
        }
        ...
        this.emitException( 'headersexception', {
            isupload: isUpload,
            msg: 'req.headers[..] not found, or HTTP method not handled',
            fatal: true
        }, 0 );
...
fproto.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
            try{
                ...
                this.emitException( 'streamexception', {
                    isupload: isUpload,
                    msg: emsg,
                    fatal: true
                }, 0 );
        ...
        copyBuffer = ( function( sourceBuffer, targetBuffer, tStart, dStart, dEnd ){
            try{
                ...
                this.emitException( 'bufferexception', {
                    isupload: isUpload,
                    msg: emsg,
                    fatal: true
                }, 0 );
        ...
        addToIncompleteList = ( function( file ){
            ...
            if( incomplete.list.indexOf( file.path ) < 0 ){
                ...
                //this.emitException( 'warning', this.uploadRootDir, 'warning <-> max upload size exceeded, this file is incomplete: ' + path.basename( file.path ), 1 ); 
                this.emitException( 'warning', {
                    isupload: true,
                    msg: 'upload threshold exceeded, file incomplete: ' + path.basename( file.path ),
                }, 1 );  
            }
        } ).bind( this ),
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                if( this.completedFiles.indexOf( filepath ) > -1 ){ 
                    ...
                    //this.emitException( 'warning', true, 'warning <-> this (sha1sum) filename already exists in the data stream: ' + sha1filename + ', orig filename: ' + escapedFilename + ', new (sha1sum) filename: ' + path.basename( filepath ), 1 );
                    this.emitException( 'warning', {
                        isupload: true,
                        msg: 'this (sha1sum) filename already exists in the data stream: ' + sha1filename + ', orig filename: ' + escapedFilename + ', new (sha1sum) filename: ' + path.basename( filepath )
                    }, 1 );
                }
        ...
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            for( var i = 0, ufile = this.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[ 0 ]; i < len; i++, currfile = ufile[ i ] ){
                fs.unlink( currfile, (function( err, cfile, i, len ){
                    if( err ){
                        //this.emitException( 'warning', isUpload, 'warning <-> file unlink exception:' + cfile + ', directory: ' + this.uploadRootDir, 1 );
                        this.emitException( 'warning', {
                            isupload: isUpload,
                            msg: 'file unlink exception:' + cfile + ', directory: ' + this.uploadRootDir
                        }, 1 );
                    }else{
