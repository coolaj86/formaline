    ...
    formaline = function ( config ) {
        ...
        var me = this,
            ...
            privateCfg = {
                ...
                res : null,
                sid : null,
                firstChunk : null, // TODO for async way
...
fproto.parse = function ( req, res, next ) {
    ...
    req.on( 'data', function ( chunk ) { 
      me.firstChunk = chunk; 
      req.pause();
      // console.log( chunk, me.firstChunk );
    } );
                    ...
                    /**/
                        fs.mkdir( me.uploadRootDir, '0750', function ( dirErr ) {
                            if ( dirErr ){
                                ...
                                if ( typeof cback === 'function' ) { cback.bind( me )(); }
                            }
                        } );
                    /**/
                    } else {
        ...
        startParsing = function () {
            ...
            if ( ctype ) {
                ...
                if ( isUpload ) {
                    ...
                    me.req.on( 'close', me.closeConnection.bind( me ) );
                    me.req.on( 'data', me.parseMultipartData.bind( me ) );
                    me.req.on( 'end', me.sendResponseToMultipart.bind( me, clength ) );
                    if ( me.req.complete ) {
                        me.parseMultipartData( me.firstChunk, me.sendResponseToMultipart.bind( me, me.firstChunk.length ) );
                    } else if ( me.firstChunk !== null ) {
                        me.parseMultipartData( me.firstChunk );
                        me.firstChunk = null;
                    }
                } else if ( isUrlEncoded ) { // TODO ASYNC
                    // seralized fields
                    me.req.on( 'close', me.closeConnection.bind( me ) );
                    me.req.on( 'data', me.parseUrlEncodedData.bind( me ) );
                    me.req.on( 'end', me.sendResponseToUrlEncoded.bind( me ) );
                    if ( me.req.complete ) {
                        me.parseUrlEncodedData( me.firstChunk, me.sendResponseToUrlEncoded.bind( me ) );
                    } else if ( me.firstChunk !== null ) {
                        me.parseUrlEncodedData( me.firstChunk );
                        me.firstChunk = null;
                    }
                } else {
    ...
    if ( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ) {
        ...
        createUploadDir( startParsing.bind( me ) );
    } else {
...
fproto.closeConnection = function( cerr ){
    if ( cerr ) {
        ...
        me.req.removeAllListeners( 'data' ); // TODO check!!
...
fproto.parseMultipartData = function( chunk, cback ){
    var me = this,
        hchunk = null; // for chopped headers
    me.req.pause();
            ...
            try {
                ...
                if ( dataPayload ) {
                    ...
                    if ( me.emitFileProgress ) {
                        ...
                        me.emitEvent( 'fileprogress', jsonFileProgression, 4, me.dataPayload );
                    }
                } else {
                    if ( cfg && cfg.path ) {
    ...
    for ( var i = 0; i < resultsLength; i++ ) {
        ...
        // me.logger( 3, '*' + hbuffer.toString() + '*' ); // TODO
