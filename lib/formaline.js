...
fproto.emitEvent = function( etype, obj, logLevel ){
    ...
    if( etype === 'end' ){
        ...
        if( obj.fatal === true ) {
            this.emit( 'end', { stats: {}, incomplete: [], completed: [] }, this.res, this.next ); // TODO add current complete/incomplete files
        }
    }else{
        this.emit( etype, obj );
    }
...
fproto.sendResponseToUrlEncoded = function(){ // add stats for fields
    var fields = querystring.parse( this.qsBuffer, '&', '=' );
    for( var f in fields ){
        var jsonFieldReceived = { name: f, value: fields[ f ] };
        this.receivedFields.list.push( jsonFieldReceived );
        this.emitEvent( 'field', jsonFieldReceived, 2 );
    }
    this.emitEvent( 'end', { stats: {}, incomplete: [], completed: [], fields: this.receivedFields.list }, 2 ); //TODO add stats
};
...
fproto.sendResponseToMultipart =  function( nbytes ){                 
    ...
    if( this.removeIncompleteFiles === false ){
        sendResponse( { stats: this.endResults, incomplete: this.incompleteFilesCollection.list, completed: this.receivedFiles, fields: this.receivedFields.list } );
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            sendResponse( { stats: this.endResults, incomplete: [], completed: this.receivedFiles, fields: this.receivedFields.list } );
        }else{
