...
fproto.emitEvent = function ( type, obj, logLevel ) {
    ...
    me.logger( logLevel, '\033[1;33mformaline, event \033[0m\033[1;3' + ( ( ( etype === 'abort' ) || ( etype === 'timeout' ) ) ? '1' : '2' ) + 'm"' + etype + '" --> \033[0m\033[0;33m', JSON.stringify( obj ), '\033[0m' );
        // etype = ( type === 'aborted' ) ? type.replace( 'aborted', 'abort' ) : type.replace( 'exception', '' ); // transform error type string
    // me.logger( logLevel, '\033[1;33mformaline, event \033[0m\033[1;32m"' + etype + '" --> \033[0m\033[0;33m', JSON.stringify( obj ), '\033[0m' );
    ...
    if ( err ) {
        ...
        if ( obj.fatal === true ) {
              if ( me.req && me.resumeRequestOnError ) {
                ...
                // TODO 
                // add current completed / incomplete files 
...
fproto.closeConnection = function( cerr ){
    if ( cerr ) {
        ...
        // me.emitEvent( cerr.code , jsonConnectionErr, 0 ); // TODO move before loadend
    }
};
