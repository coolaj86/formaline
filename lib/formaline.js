...
FPROTO.parse = function( req, res, next ){
    ...
    var hs = req.headers,
        ...
        bytes2Receive = 0,
        isUpload =  ( ctype && ( ctype.indexOf( 'multipart/form-data' ) > -1 ) ) ? true : false ;//false;
    
    this.startTime = new Date().getTime();
        
    this.next = ( next && ( typeof next === 'function') ) ? next : emptyFn;//try to add connect layer compatibility
    
    this.emitException = function( type, isUpload, msg ){
        this.emit( type, isUpload, 'formaline, ' + msg, res, this.next );
        this.emit( 'end', [], {}, res, this.next );
    };
    
    this.getProgressEmitter = function( headerContentLength ){
       ...
       ( req.headers ) && ( req.headers['content-type'] ) && ( ~req.headers['content-type'].indexOf( 'multipart/form-data' ) || ~req.headers['content-type'].indexOf( 'urlencoded' ) ) ){
        ...
        try{ //try to create a sub-directory of root upload directory
            ...
            try{
                ...
                this.logger( 0, '\nformaline, default mkDir Error-->', this.uploadRootDir,' message:', ierr.message );
                this.emitException( 'pathexception', this.uploadRootDir, ierr.message );
        ...
        this.dataProgress = this.getProgressEmitter( clength );
        /** /
        ...
        }).bind( this, this.req.headers['content-length'] )();
      ...
      /**/
            ...
            try{
                ...
                if( bytes2Receive > this.uploadThreshold ){//check Max Upload Size in bytes
                    if( this.checkContentLength === true ){
                        this.emitException( 'headersexception', isUpload, 'formaline, req.headers[content-length] exceeds max allowable: ' + bytes2Receive + ' > ' + this.uploadThreshold );
                        return;
                    }
                    //warning
                    this.logger( 1, ' formaline, req.headers[content-length] --> Content Length Warning, bytes to receive:', bytes2Receive, 'max allowed:', this.uploadThreshold );
                    this.emit('warning', 'formaline, Content Length Warning, bytes to receive: '+bytes2Receive+', allowed: '+this.uploadThreshold);
                }else{
                    this.logger( 0, ' formaline, req.headers[content-length] --> ', bytes2Receive, 'bytes' );
                }
            }catch(error){
                //error
                this.logger( 0, ' formaline, req.headers[content-length] --> Parse Length Exception', error );
                this.emitException( 'headersexception', isUpload , 'req.headers[content-length] parse error -> value is: ' + clength );
                return;
            }
        }else{
            //error
            this.logger( 0, ' formaline, req.headers[content-length] --> Parse Length Error' );
            this.emitException( 'headersexception', isUpload , 'req.headers[content-length] not defined' );
            return;
        }
        ...
        if( ctype ){
            ...
            if( ~ctype.indexOf( 'multipart/form-data' ) ){
                ...
                this.logger(1,' boundary pattern length:'+this.boundString.length+'\n');
                
                this.req.addListener('data', this.parseData.bind( this ) );
                this.req.addListener( 'end', this.sendResponse.bind( this, clength ) );
                ...
                if( bytes2Receive > this.uploadThreshold ){
                    if( this.checkContentLength === true ){
                        this.emitException( 'headersexception', false, 'req.headers[content-length] exceeds max allowable: ' + bytes2Receive + ' > ' + this.uploadThreshold );
                        return;
                    } else {
                        //warning
                        ...
                        this.emit('warning', 'formaline, Content Length Warning, bytes to receive: '+bytes2Receive+', bytes allowed: '+this.uploadThreshold);
                    }
                } 
                ...
                this.emitException('headersexception', isUpload, 'req.headers[content-type] --> ' + ctype + ' handler is not defined' );
                return;
            }
        }else{
            //error
            this.logger(0,'\n formaline, req.headers[content-type] --> Parse Type Error');            
            this.emitException( 'headersexception', isUpload, 'req.headers[content-type] is not defined' );
            return;
        }
    }else{
        //error
        this.logger(0,'\n formaline, req.headers[..] --> No headers Error');
        this.emitException( 'headersexception', isUpload, 'req.headers[..] are not defined' );
        return;
    }
...
FPROTO.parseData = function(chunk){
                    ...
                    if( this.bytesReceived <= this.uploadThreshold ){// is size allowed? 
                        if( this.chopped && this.fileStream ){// file data is chopped? && fileStream exists?
                            if( resultsLength === 0 ){//chunk is only data payload
                                ...
                                if( ( fileData.length > 0 ) && ( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ) ){
                                    try{
                                        chunk.copy( fileData, 0, 0, fileData.length - 1 );//resolved index bug !!!  rstart - 2 );
                                        ...
                                        this.logger(0,'\nformaline, filestream or buffer data exception -->', err,'\n results length:',resultsLength,'\n \nbuffer start index:',0,'\n buffer data end index:',fileData.length-1,'\n fileData buffer length:',fileData.length,'\n fileStream:',this.fileStream,'\n err.stack:',err.stack);
                                        this.emit( 'end', [], {}, this.res, this.next); // TODO
                                        return; 
                                    }
                                ...
                                this.fileStream.end();
                                this.logger(3,'\n   ->closed file stream -->',this.fileStream.path,'\n');
                        ...
                        } // end if( this.chopped && this.fileStream ){
                        
                    }else{ // if( this.bytesReceived <= this.uploadThreshold ){
                    ...
                    for( var i = 0; i < resultsLength; i++ ){
                        ...
                        if( rfinish > rstart + bblength + 2 ){ //check the creation of heads buffer (indexes) 
                            try{
                                ...
                                chunk.copy( heads, 0, rstart + bblength + 2, ( rfinish > chunk.length - 1 ) ? ( chunk.length - 1 ) : rfinish ); 

                            }catch(err){
                                this.emit('exception', true, 'formaline, exception, copying buffer datafile --> '+err.message, this.res, this.next);
                                this.logger(0,'\nformaline, buffer datafile exception -->', err, '\nresult:',result,'\nboundary length:',bblength,'\nchunk length:',chunk.length,'\nstart buffer index:',rstart + bblength + 2,'\nend buffer index:',rfinish ,'\nheaders buffer length:', heads.length,'\nerr.stack:',err.stack);
                                this.emit( 'end', [], {}, this.res, this.next); // TODO
                        ...
                        if(fieldName){
                            ...
                            var fileName = headers.match(/filename="([^\"]+)"/mi), // headers.match(/filename="([^;]+)"/mi),
                            ...
                            if(fileName) { // file field
                                ...
                                if( this.completedFiles.indexOf(filepath) < 0 ){ //file with the same name already exists
                                    ...
                                    this.fileStream.addListener("error", function(err) {
                                       //console.log('\n\nGot error while writing to file :', err+'\n\n');
                                      this.logger(0,'\nformaline, got error while writing to file : -->', err, '\nfile stream:', this.fileStream,'\nerr.stack:',err.stack);
                                    });
                                ...
                                this.fileStream.ctype = fieldCtype;
                                            ...
                                            if( this.bytesReceived <= this.uploadThreshold ){
                                                if( chunkLength >= rfinish + 4 ){
                                                    try{
                                                        ...
                                                        this.logger(0,'\nformaline, buffer datafile exception -->', err, '\nfileData buffer length:',fileData.length,'\nstart buffer index:',rfinish + 4,'\nend buffer index:',chunkLength ,'\nerr.stack:',err.stack);
                                    ...
                                    if(this.fileStream){
                                        ...
                                        if( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ){      
                                            try{
                                                ...
                                                this.emit('exception', true, 'formaline,exception, filestream copying buffer datafile : '+this.fileStream.path+' --> '+err.message, this.res, this.next);
                                                this.logger(0,'\nformaline, filestream buffer data exception -->', err,'\nresults length:',resultsLength,'\nstart buffer index:',results[i].finish + 4,'\nend buffer index:',results[i+1].start - 2 ,'\nfileData buffer length:',fileData.length);
                                                return; 
                                            }
                                            this.fileSize += fileData.length;
                                            this.bytesWrittenToDisk += fileData.length;
                                        ...
                                        if( ( this.fileSize >= 0 ) && (this.incompleteFilesCollection.list.indexOf(this.fileStream.path) < 0 ) ){
                                            ...
                                            this.emit( 'filereceived', path.basename( this.fileStream.path ), this.fileStream.origname, path.dirname( this.fileStream.path ), fieldCtype, this.fileSize, this.fileStream.fieldname,( ( this.sha1sum ) ? this.fileStream.sha1sum.digest('hex') : undefined ) );
                                        }
                                        this.fileStream.end();
                                        this.logger(3,'\n   ->closed file stream -->',this.fileStream.path,'\n');
                                        //reset fileStream
                                ...
                                if( i < resultsLength - 1 ){
                                    try{
                                        ...
                                        this.logger(0,'\nformaline, copyind data buffer exception -->', err);
                                        this.emit( 'end', [], {}, this.res, this.next); // TODO
                                        return;
                                    }
                                    ...
                                    this.logger(3,'   ->field:',fieldName[1]+', content-type: '+ fieldCtype ,'data: *',fileData.toString(),'*');//is many times null for field                                   
...
FPROTO.sendResponse =  function(nbytes){                 
    this.endTime = new Date().getTime();
    this.endResults = {
...
FPROTO.handleXHR =  function( headers ){
