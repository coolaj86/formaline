...
fproto.sendResponseToMultipart =  function( nbytes ){                 
    ...
    if( this.removeIncompleteFiles === false ){
        sendResponse( { stats: this.endResults, incomplete: this.incompleteFilesCollection.list, completed: this.receivedFiles } );
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            sendResponse( { stats: this.endResults, incomplete: [], completed: this.receivedFiles } );
        }else{
