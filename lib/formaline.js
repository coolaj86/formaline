...
var setDebuggingLevel = function( dstring, form ){
    var p = '', 
        dlevels = querystring.parse( "debug:off,1:on,2:on,3:on", ',', ':' ), // debug:'on' print always : 0 level
        flog = null,
        fpath = null
        filelogging = false;
    if( dstring ){
    ...
    if( filelogging ){
        form.on( 'startlogging', ( function( req ){
            fpath = form.uploadRootDir + path.basename( form.uploadRootDir ).replace( '/', '' ) + '.log';
            flog = new fs.WriteStream( fpath );
            console.log( '\nformaline \'startlogging\': new log file created ->', fpath, '\n');
        } ).bind( this ) );
        
        form.on( 'stoplogging', ( function( req ){
            flog.end();
            console.log( '\nformaline \'stoplogging\': log file closed ->', fpath, '\n' );
            flog = null;
            fpath = null;
        } ).bind( this ) );
    }
    return function(){
        ...
        if( typeof level === 'number' ){
            if( ( level === 0 ) || ( dlevels[ level ] === 'on' )){
                if( filelogging && flog ) {
                    flog.write( JSON.stringify(args) );// )args.slice( 1, args.length, 'utf8' ) ) );
                }
...
var formaline = function ( config ){
    ...
    this.logger = setDebuggingLevel( this.logging, this ); // TODO emptyFn;  
...
fproto.parseMultipartData = function( chunk ){
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( fieldName ){
            ...
            this.logger( 3, ' fieldName parsed --> : ', fieldName, '\n' );
            if( fileName ){ // file field
                ...
                this.logger( 3, ' fileName parsed --> : ', fileName, '\n' ); 
        ...
        }else{ // end if(fieldname)
            // TODO 
           this.logger( 3, ' <-- no field name founded, resuming request.. \n' ); 
        }         
    } // end for
