...
fproto.sendResponseToUrlEncoded = function(){ // add stats for fields
    ...
    this.emitEvent( 'end', { stats: {}, incomplete: [], completed: [] }, 2 );
};
...
fproto.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
        ...
        addToCompletedList = ( function( file ){
            ...
            this.receivedFilesCollection[ jsonReceivedFile.sha1name ] = jsonReceivedFile ;
...
fproto.sendResponseToMultipart =  function( nbytes ){                 
    ...
    if( this.removeIncompleteFiles === false ){
        sendResponse( { stats: this.endResults, incomplete: this.incompleteFilesCollection.list, completed: this.receivedFilesCollection } );
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            sendResponse( { stats: this.endResults, incomplete: [], completed: this.receivedFilesCollection } );
        }else{
