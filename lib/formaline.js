...
FPROTO.parse = function( req, res, next ){
    ...
    this.validatePostSize = function( expected, isUpload ){
        if( expected > this.uploadThreshold ){ // check Max Upload Size in bytes
            if( this.checkContentLength === true ){
                return false;
            }
            //warning
            this.emitException( 'warning', isUpload, 'Content Length Warning, bytes to receive: ' + expected  + ', bytes allowed: ' + this.uploadThreshold, 1 );
        }else{
    ...
    if( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ){
        ...
        try{ 
            ...
            fs.mkdirSync( null,'750');//this.uploadRootDir, '0750' );//'0755');
        }catch( dirErr ){
            this.uploadRootDir = '/tmp/' + parseInt( new Date().getTime() * ( 1 + Math.random() ) * 10 * 32, 10 ) + '/';
            this.emitException( 'warning', this.uploadRootDir, 'Warning: default directory creation Error: ' + this.uploadRootDir + ', ' + dirErr.message, 0 );
            try{
                fs.mkdirSync( this.uploadRootDir, '0750' );//'0755'); 
            }catch( ierr ){
                this.emitException( 'pathexception', this.uploadRootDir, 'Default Dir Creation Error' + ierr.message, 0, true );
                return;
            }
        }
        ...
        if( clength ){
            ...
            try{
                ...
                //error
                this.emitException( 'headersexception', isUpload , 'req.headers[content-length]: '+ parseIntError + ', length value:' + clength, 0 );
                return;
            }
            if( ! this.validatePostSize( bytes2Receive, isUpload ) ){
            ...
            //error
            this.emitException( 'headersexception', isUpload , 'req.headers[content-length] not defined:  Parse Length Error', 0, true );
            return;
        }
        ...
        if( ctype ){
            ...
            if( isUpload ){
                ...
                //error
                this.emitException('headersexception', isUpload, 'req.headers[content-type] --> ' + ctype + ' handler for this kind of POST is not defined', 0, true );
                return;
            }
        }else{
            //error
            this.emitException( 'headersexception', isUpload, 'req.headers[content-type] is not defined: Parse Type Error', 0, true );
            return;
        }
    }else{
        //error
        this.emitException( 'headersexception', isUpload, 'req.headers[..] are not defined', 0, true );
        return;
    }
