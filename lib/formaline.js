        ...
        if( this.chopped && this.fileStream ){// file data is chopped? && fileStream exists?
            if( resultsLength === 0 ){//chunk is only data payload
                ...
                try{
                    ...
                    emsg = 'exception writing on file stream : ' + this.fileStream + ', err: ' + fserr.message ;
                    emsg +=  '\n error stack:\n  ' + fserr.stack;
                ...
                if( ( fileData.length > 0 ) && ( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ) ){
                    try{
                        ...
                        emsg = 'exception copying buffer data file: ' + err.message;
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( rfinish > rstart + bblength + 2 ){ //check the creation of heads buffer (indexes) 
            try{
                ...
                emsg = 'exception copying buffer data file: ' + err.message;
                //TODO TODO TODO!!! haeds lenght here!!!
                emsg += '\nboundary length:' + bblength + '\nchunk length:' + chunk.length;
                emsg += '\nresult:' + result + '\n results length:' + resultsLength + '\n buffer start index:' + ( rstart + bblength + 2 ) + '\n buffer data end index: ' + rfinish + '\n fileData buffer length: ' + fileData.length;
                emsg +=  '\n error stack:\n  ' + err.stack;
                this.emitException( 'exception', true, emsg, 0, true );
                return; 
            }
                    ...
                    if(this.fileStream){
                        ...
                        if( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ){      
                            try{
                                ...
                                emsg = 'exception copying buffer data file : ' + err.message;
                                emsg += '\nboundary length:' + bblength + '\nchunk length:' + chunk.length;
                                emsg += '\nresult:' + result + '\n results length:' + resultsLength + '\n buffer start index:' + ( results[ i ].finish + 4 ) + '\n buffer data end index: ' + ( results[ i + 1 ].start - 2 ) + '\n fileData buffer length: ' + fileData.length;
                                emsg +=  '\n error stack:\n  ' + err.stack;
                                this.emitException( 'exception', true, emsg, 0, true );
                                return; 
