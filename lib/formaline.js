...
fproto.emitEvent = function( etype, obj, logLevel ){
    ...
    }else if( ( etype.indexOf('warning') > - 1 ) || ( etype.indexOf('exception') > - 1 ) ){ // exception event
        obj.type = etype;
...
fproto.sendResponseToMultipart =  function( nbytes ){                 
    ...
    if( this.removeIncompleteFiles === false ){
        ...
        this.emitEvent( 'end', { stats: this.endResults, incomplete: this.incompleteFilesCollection.list }, 2 );
        resetAttributes();
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            this.emitEvent( 'end', { stats: this.endResults, incomplete: [] }, 2 );
            resetAttributes();
        }else{
