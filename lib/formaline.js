...
fproto._emitException = function( type, isUpload, msg, logLevel, endResponse ){
...
fproto._emitField = function( fieldname, fieldvalue ){
        this.logger( 2, '\n formaline, field <-> name: "' + fieldname + '", value: "' + fieldvalue +'"' );
        this.emit( 'field', fieldname, fieldvalue);
};
        
fproto._emitFileReceived = function( sha1name, origname, path, type, size, fieldname, datasha1sum ){
        this.logger( 2, '\n formaline, filereceived <-> sha1name: ' + sha1name + ', original name: ' +  origname + ', path: ' + path + ', type: ' + type + ', bytes: ' +  size + ', field: ' + fieldname + ', data sha1 sum: ' + datasha1sum + '\n' );
        this.emit( 'filereceived', sha1name, origname, path , type, size, fieldname, datasha1sum );    
};
    
fproto._emitFileRemoved = function( sha1name, origname, path, type, size, fieldname ){
       this.logger( 2, '\n formaline, fileremoved <-> sha1name: ' + sha1name + ', original name: ' +  origname + ', path: ' + path + ', type: ' + type + ', bytes: ' + size + ', field: ' + fieldname + ', data sha1 sum: not calculated \n' );
        this.emit( 'fileremoved', sha1name, origname, path , type, size, fieldname );    
};
     
fproto._emitProgress = function( bytes, chunks, ratio ){
        this.logger( 3, '\n formaline, dataprogress <->  bytes:', bytes, 'chunks:', chunks, ' ratio:', ratio ); 
        this.emit( 'dataprogress', bytes, chunks, ratio );
};
...
fproto.parse = function( req, res, next ){
    ...
    var hs = req.headers,
        ...
        getProgressEmitter = ( function( headerContentLength ){
            ...
            if( dProgress === true ){
                return function( isEnd ){    
                    this._emitProgress( this.bytesReceived, this.chunksReceived, ratio( this.bytesReceived ) ); // every chunk
                };
                ...
                if( dProgress < 2 ){ dProgress = 2; }
                    return function( isEnd) {
                        if( ( ( this.chunksReceived % dProgress ) === 1 ) || isEnd ){ // mod 1 is for first chunk
                           this._emitProgress( this.bytesReceived, this.chunksReceived, ratio( this.bytesReceived ) ); //  every dProgress chunks 
                        }
                    };
            }else{
                return emptyFn;
        ...
        validatePostSize = ( function( expected, isUpload ){
            ...
            if( expected > this.uploadThreshold ){ // check Max Upload Size in bytes
                ...
                this._emitException( 'warning', isUpload, 'warning <-> invalid content-length header, bytes to receive: ' + expected  + ', bytes allowed: ' + this.uploadThreshold, 1 );
            }
            return true;
        } ).bind( this ),
            ...
            try{
                if( typeof this.getSessionID === 'function' ){
                    ...
                    if( typeof sessionID !== 'string' ){
                        this._emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier, function \'this.getSessionID( req )\' doesn\'t return a String!', 1 );        
                    }else{
                    ...
                    this._emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier, configuration parameter \'this.getSessionID\' must be a function!', 1 );

                }  
              }catch( serr ){
                  this._emitException( 'warning', isUpload, 'warning <-> unable to retrieve session identifier: ' + serr.stack, 1 );
                  return null;
              }
    ...
    if( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ){
        ...
        try{ 
            ...
            this._emitException( 'warning', isUpload, 'warning <-> default directory creation exception: ' + this.uploadRootDir + ', ' + dirErr.message, 0 );
            try{
                fs.mkdirSync( this.uploadRootDir, '0750' ); 
            }catch( ierr ){
                this._emitException( 'pathexception', isUpload, 'directory creation exception: '+ this.uploadRootDir + ', ' + ierr.message, 0, true );
                return;
            }
        }
        ...
        try{
            ...
            if( isUpload ){
                ...
                this._emitException( 'headersexception', isUpload , 'req.headers[..]: the multipart/form-data request is not HTTP-compliant, boundary string wasn\'t found..', 0, true );
                return;
            }
        }
        ...
        if( clength ){ 
            try{
                ...
                this._emitException( 'headersexception', isUpload , 'req.headers[ content-length ]: '+ parseIntError + ', length value:' + clength, 0, true );
                return;
            }
            if( ! validatePostSize( bytes2Receive, isUpload ) ){
                this._emitException( 'headersexception', isUpload, 'req.headers[ content-length ] exceeds max allowable: ' + bytes2Receive + ' > ' + this.uploadThreshold, 0, true );
                return;
            }
        }else{
            this._emitException( 'headersexception', isUpload , 'req.headers[ content-length ] not found: Parse Length Error', 0, true );
            return;
        }
        if( ctype ){   
            ...
            if( isUpload ){
                ...
                this._emitException( 'headersexception', isUpload, 'req.headers[ content-type ] --> ' + ctype + ' handler for this kind of request is not defined', 0, true );
                return;
            }
        }else{
            this._emitException( 'headersexception', isUpload, 'req.headers[ content-type ] not found: Parse Type Error', 0, true );
            return;
        }
    }else{
        this._emitException( 'headersexception', isUpload, 'req.headers[..] not found, or HTTP method not handled ', 0, true );
        return;
    }
...
fproto.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
            try{
                ...
                emsg +=  '\n error stack:\n  ' + fserr.stack;
                this._emitException( 'streamexception', true, emsg, 0, true );
                return;
            }
        } ).bind( this ),
        ...
        copyBuffer = ( function( sourceBuffer, targetBuffer, tStart, dStart, dEnd ){
            try{
                ...
                emsg +=  '\n error stack:\n  ' + berr.stack;
                this._emitException( 'bufferexception', true, emsg, 0, true );
                return; 
            }
        } ).bind( this ),
        ...
        addToIncompleteList = ( function( file ){
            ...
            if( incomplete.list.indexOf( file.path ) < 0 ){
                ...
                this._emitException( 'warning', this.uploadRootDir, 'warning <-> max upload size exceeded, this file is incomplete: ' + path.basename( file.path ), 1 );
            }
        } ).bind( this ),
        ...
        addToCompletedList = ( function( file ){
            ...
            this._emitFileReceived( path.basename( file.path ), file.origname, path.dirname( file.path ), file.ctype, this.fileSize, file.fieldname, filedatasha1sum );
        } ).bind( this ),
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                if( this.completedFiles.indexOf( filepath ) > -1 ){ 
                    ...
                    this._emitException( 'warning', true, 'warning <-> this (sha1sum) filename already exists in the data stream: ' + sha1filename + ', orig filename: ' + escapedFilename + ', new (sha1sum) filename: ' + path.basename( filepath ), 1 );
                }
                ...
                if( i < resultsLength - 1 ){
                    fileDataChunk = new Buffer( results[ i + 1 ].start - 2   - ( results[ i ].finish + 4 ) ); // + 4 ( CRLFCRLF )
                    copyBuffer( chunk, fileDataChunk, 0, results[ i ].finish + 4 , results[ i + 1 ].start - 2 ); 
                    this._emitField( fieldName[ 1 ], fileDataChunk.toString() );
                }
            }
        } // end if(fieldname)         
    } // end for
        ...
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            for( var i = 0, ufile = this.incompleteFilesCollection.list, len = ufile.length, currfile = ufile[ 0 ]; i < len; i++, currfile = ufile[ i ] ){
                fs.unlink( currfile, (function( err, cfile, i, len ){
                    if( err ){
                        this._emitException( 'warning', isUpload, 'warning <-> file unlink exception:' + cfile + ', directory: ' + this.uploadRootDir, 1 );
                    }else{
                         //this.logger( 0, '\nformaline, incomplete file removed -->', cfile );
                         var ifile = this.incompleteFilesCollection[ path.basename( cfile ) ];
                         this._emitFileRemoved( 2, path.basename( cfile ), ifile.origname, ifile.dir, ifile.type, ifile.rbytes, ifile.field );
                    }
                    if( i === len - 1){
                        logParserStats();
