...
fproto.parseMultipartData = function( chunk ){
    ...
    //this.logger( 3, ' parser results -->', results, ' \n' );
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( headers !== '' ){ this.logger( 3, ' headers: ' + headers + '\n' ); }
        ...
        if( fieldName ){ 
            ...
            var tbsize = chunk.length - rstart,// + 1,
                tbuff = new Buffer( tbsize ),
                endBoundary = '--' + this.boundString + '--';
            
            chunk.copy( tbuff, 0, rstart );
            console.log( tbuff.length, chunk.length - rstart );
            console.log( rstart, chunk.length, tbsize );
            if ( tbuff.toString().indexOf( endBoundary ) === -1 ){
                // headers are chopped in two different chunks
                // not handling this situation brings this file/field to disappear
                ...
                console.log( '\n header is chopped *' + tbuff.toString() + '*\n' );
            }else{
                this.logger( 3, ' <-- no field name was found.. end of the request reached \n' );
                //console.log( '\n end request --> *' + tbuff.toString() + '*\n' );  
            }
            //this.logger( 3, 'chunk length:' + chunk.length + ', payload start index: ' + ( rstart + bblength + 2 ) + '\n' );
        }         
    } // end for
