...
var formaline = function ( config ){
    ...
    this.emitField = ( function( fieldname, fieldvalue ){
        this.logger( 3, '\nformaline, field  event->  name:', fieldname, 'value:', fieldvalue );
        this.emit( 'field', fieldname, fieldvalue);
    } ).bind( this ),
        
    this.emitFile = ( function( ){
            
    } ).bind( this ),
     
    this.emitProgress = ( function( bytes, chunks, ratio ){
        this.logger( 3, '\nformaline, dataprogress event->  bytes:', bytes, 'chunks:', chunks, ' ratio:', ratio ); 
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                this.logger( 2,' -> field: ' + fieldName[ 1 ] + ', original filename: ' + escapedFilename + ', new (sha1sum) filename: ' + sha1filename + ', content-type: ' + fieldCtype );
                ...
                if( i < resultsLength - 1 ){
                    ...
                    copyBuffer( chunk, fileDataChunk, 0, results[ i ].finish + 4 , results[ i + 1 ].start - 2 ); 
                  //TODO
                    //emitField( fieldName[ 1 ], fileDataChunk.toString() );
