...
fproto.parse = function ( req, res, next ) {
    ...
    var hs = req.headers,
        ...
        startParsing = function () {
            ...
            if ( ctype ) {
                if ( isUpload ) { // TODO minify code
                    // multipart form data
                    me.boundBuffer = new Buffer( '--' + me.boundString );
                    // me.logger( 1, 'formaline, boundary length: ' + me.boundBuffer.length + ' bytes' );
                    req.on( 'close', me.closeConnection.bind( me ) );
                    req.on( 'data', me.parseMultipartData.bind( me ) );
                    req.on( 'end', me.sendResponseToMultipart.bind( me ) );
                    if ( req.complete ) {
                        if ( me.firstChunk !== null ) { 
                            me.parseMultipartData( me.firstChunk, me.sendResponseToMultipart.bind( me ) );
                        } else { // empty request !
                            // error, no data
                            // notify and send response, or wait for timeout?
                            me.emitEvent( 'message', jsonWarnEmptyRequest, 1 );
                            me.sendResponseToMultipart.bind( me )();
                        }
                    } else {
                        if ( me.firstChunk !== null ) {
                ...
                } else if ( isUrlEncoded ) { // TODO minify code
                    // seralized fields
                    req.on( 'close', me.closeConnection.bind( me ) );
                    req.on( 'data', me.parseUrlEncodedData.bind( me ) );
                    req.on( 'end', me.sendResponseToUrlEncoded.bind( me ) );
                    if ( req.complete ) {
                        if ( me.firstChunk !== null ) {
                            ...
                            // notify and send response or wait for timeout?
                            me.emitEvent( 'message', jsonWarnEmptyRequest, 1 );
                            me.sendResponseToUrlEncoded.bind( me )();
                        }
                    } else {
                        if ( me.firstChunk !== null ) {
