            ...
            if ( typeof level === 'number' ) {
                if ( ( level === 0 ) || ( dlevels[ level ] === 'on' ) ) {
                    ...
                    if ( dlevels[ 'console' ] === 'on' ) {
                        ...
                        //sargs.unshift( cc[ dc[ ( level ) ? level : 0 ] ] );
                        //sargs.splice( 2, 0, green[ 1 ] );
                        //console.log( level, dc[ level ])
                        sargs.unshift( cc[ dc[ level ] ] );
                        sargs.push( '\033[0m' );
                        return console.log.apply( this, sargs );                      
                        /** /
                        //console.log( cc[ dc[ level ] ] );
...
fproto.emitEvent = function ( type, obj, logLevel ) {
    var me = this,
        //color = [ '\033[1;42m', '\033[0m' ],
        etype = ( type === 'aborted' ) ? type.replace( 'aborted', 'abort' ) : type.replace( 'exception', '' ); // transform error type string
    me.logger( logLevel, '\033[1;33mformaline, event \033[0m\033[1;32m"' + etype + '" --> \033[0m\033[1m\033[0;33m', JSON.stringify( obj ), '\033[0m' );
...
fproto.parseMultipartData = function( chunk ){
    ...
    me.req.pause();
    me.logger( 3, '\033[32mformaline, new data chunk received, request paused ..\033[0m ' );
    ...
    var writeToFileStream = function ( dataPayload, cfg ) {
            try {
                if ( dataPayload ) {
                    ...
                    me.logger( 3, '\033[37mformaline, new data were written to this file stream  --> \033[34m', me.fileStream.path, '\033[0m' );
                    ...
                    if ( cfg && cfg.path ) {
                        ...
                        me.logger( 3, '\033[7;37mformaline, a new file stream was created --> \033[34m', me.fileStream.path, '\033[0m' );
                    } else {
    ...
    me.logger( 3, '\033[32mformaline, resuming request ..\033[0m' ); // TODO remove from last call
    me.req.resume();
};
