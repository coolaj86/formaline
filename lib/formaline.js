...
var formaline = function ( config ){
    ...
    this.resumeRequestOnFatalException = true; 
    ...
    this.receivedFiles = {};  // object properties are unique (sha1 sum filename)
...
fproto.emitEvent = function( etype, obj, logLevel ){
    ...
    if( etype === 'end' ){
        ...
        if( obj.fatal === true ) {
            ...
            // TODO TODO TODO check with copyBuffer, add stats, add logging
            if( this.req && this.resumeRequestOnFatalException ){ // on exception resume request and remove 'data' event listener 
                this.req.removeAllListeners( 'data' );
                this.req.resume();
            }else{
                // TODO add current complete/incomplete files ?
                this.emit( 'end', { stats: {}, incomplete: [], completed: [] }, this.res, this.next );
            }
        }
    }else{
        this.emit( etype, obj );
...
fproto.parse = function( req, res, next ){
    ...
    this.req.socket.setTimeout( this.requestTimeOut ); // TODO TODO
    ...
    if( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ){
        ...
        var jsonFatalErr = { isupload: isUpload, msg:'', fatal: true },
            jsonWarnErr = { isupload: isUpload, msg:'', fatal: false };
        ...
        if( path.existsSync( this.uploadRootDir ) ){
            ...
            if( this.uploadRootDir === '/tmp/' ){
                ...
                this.emitEvent( 'warning', jsonWarnErr, 1 );
        ...
        if( !path.existsSync( this.uploadRootDir ) ){ // if subdirectory doesn't already exist, create it
             try{
                 ...
                 this.emitEvent( 'direxception', jsonFatalErr, 0 );
                 return;
             }
        }else{
...
fproto.closeConnection = function( cerr, isUpload ){
    ...
    this.emitEvent( cerr.code + 'exception :', jsonConnectionErr, 0 );
};
...
fproto.sendResponseToUrlEncoded = function(){ // TODO add stats for fields
    ...
    this.emitEvent( 'end', { stats: {}, incomplete: [], completed: [], fields: this.receivedFieldsCollection.list }, 2 ); 
};
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
            try{
                if( dataPayload ){
                    ...
                    var fpath = this.fileStream.path;
                    if( fpath ){
                        this.fileStream = new fs.WriteStream( fpath );                        
        ...
        addToCompletedList = ( function( file ){
            ...
            this.receivedFiles[ jsonReceivedFile.hashname ] = jsonReceivedFile ;
            this.emitEvent( 'filereceived', jsonReceivedFile, 2 );
        } ).bind( this ),
        ...
        closeFileStream = ( function( fstream ){
            ...
            this.logger( 3, '\nformaline, closed file stream -->', fstream.path, '\n' );
        } ).bind( this ),
...
fproto.sendResponseToMultipart =  function( nbytes ){                 
    ...
    if( this.removeIncompleteFiles === false ){
        sendResponse( { stats: this.endResults, incomplete: this.incompleteFilesCollection.list, completed: this.receivedFiles, fields: this.receivedFieldsCollection.list } );
    }else{
        if( this.incompleteFilesCollection.list.length === 0 ){
            ...
            sendResponse( { stats: this.endResults, incomplete: [], completed: this.receivedFiles, fields: this.receivedFieldsCollection.list } );
        }else{
