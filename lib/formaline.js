    ...
    if( ( req ) && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' )  && ( hs ) ){
        ...
        try{ 
            ...
            try{
                ...
                this.emitException( 'pathexception', this.uploadRootDir, 'directory creation Error:' + ierr.message, 0, true );
                return;
            }
        }
...
FPROTO.parseMultipartData = function( chunk ){
    ...
    // WRITE BUFFERS
    this.writeToFileStream = function( dataPayload, filePath ){  
        try{
            if( dataPayload ){
    ...
    // COPY BUFFERS
    this.copyBuffer = function( sourceBuffer, targetBuffer, tStart, dStart, dEnd ){
        try{
            sourceBuffer.copy( targetBuffer, tStart, dStart, dEnd ); // resolved index bug !!!  rstart - 2 );
        }catch( berr ){
    ...
    this.addToIncompleteList = function( file ){
        if( this.incompleteFilesCollection.list.indexOf( file.path ) < 0 ){
            this.incompleteFilesCollection.list.push( file.path );
            this.incompleteFilesCollection[ path.basename( file.path ) ] = {
    ...
    this.addToCompletedList = function( file ){
    ...
    this.closeFileStream = function( file ){
        file.end();
        this.logger( 3, '\n -> closed file stream -->', file.path, '\n' );
    };
    
    this.resetFileStream = function( ){
        this.fileStream = null;
    };
    ...
    if( this.bytesReceived <= this.uploadThreshold ){ // is size allowed? 
        if( this.chopped && this.fileStream ){ // file data is chopped? && fileStream exists?
            if( resultsLength === 0 ){ // chunk is only data payload
                ...
                if( ( fileData.length > 0 ) && ( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ) ){
                    this.copyBuffer( chunk, fileData, 0, 0, fileData.length - 1 );
                    this.writeToFileStream( fileData );
                }
                
                this.closeFileStream( this.fileStream );
                //this.fileStream.end();
                //this.logger( 3, '\n  -> closed file stream -->' , this.fileStream.path, '\n' );
                

                
                this.addToCompletedList( this.fileStream );
                /** /
        ...
        this.addToIncompleteList( this.fileStream );
    }
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( rfinish > rstart + bblength + 2 ){ //check the creation of heads buffer (indexes) 
            this.copyBuffer( chunk, heads, 0, rstart + bblength + 2,  ( rfinish > chunk.length - 1 ) ? ( chunk.length - 1 ) : rfinish );
        }//end if                  
        ...
        if( fieldName ){
            ...
            if(fileName) { // file field
                ...
                if( this.completedFiles.indexOf( filepath ) < 0 ){ //file with the same name already exists
                    this.writeToFileStream( null, filepath );
                }else{
                ...
                if( i === resultsLength - 1 ) { // last result
                        ...
                        if( this.fileStream ){
                            if( this.bytesReceived <= this.uploadThreshold ){
                                if( chunkLength >= rfinish + 4 ){
                                    var fileData = new Buffer( chunkLength - ( rfinish + 4  ) ); 
                                    this.copyBuffer( chunk, fileData, 0, rfinish + 4, chunkLength );                                        
                                    this.writeToFileStream( fileData );
                                }
                            }else{
                                this.addToIncompleteList( this.fileStream );
                    ...
                    if( this.fileStream ){
                        ...
                        if( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ){      
                            this.copyBuffer( chunk, fileData, 0, results[ i ].finish + 4 , results[ i + 1 ].start - 2 ); 
                            this.writeToFileStream( fileData );
                        }else{
                            this.addToIncompleteList( this.fileStream );
                        }
                ...
                if( i < resultsLength - 1 ){
                    ...
                    this.logger( 3,' -> field:', fieldName[ 1 ] + ', content-type: ' + fieldCtype , 'data: *', fileData.toString(), '*' );
