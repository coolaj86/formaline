...
FPROTO.parse = function( req, res, next ){
    ...
    this.validatePostSize = function(expected){
        if( expected > this.uploadThreshold ){ // check Max Upload Size in bytes
            if( this.checkContentLength === true ){
                return false;
            }
            //warning
            this.logger( 1, ' formaline, req.headers[content-length] --> Content Length Warning, bytes to receive:', expected, 'max allowed:', this.uploadThreshold );
            this.emit( 'warning', 'formaline, Content Length Warning, bytes to receive: ' + expected  + ', bytes allowed: ' + this.uploadThreshold );
        }else{
             this.logger( 0, ' formaline, req.headers[content-length] --> ', expected , 'bytes' );
        }
        return true;
    };
    ...
    if( req && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' ) && 
        ( hs ) && ( ctype ) && (  isUpload || isUrlEncoded ) ){
        this.req = req;
        this.res = res;
        ...
        try{ 
            //try to create a sub-directory of root upload directory
            ...
            this.logger( 0, '\nformaline, mkDir Warning-->', this.uploadRootDir, 'message:', oerr.message );
            this.emit( 'warning', oerr.message );
            try{
                fs.mkdirSync( this.uploadRootDir, '0750' );//'0755'); 
            }catch( ierr ){
                this.logger( 0, '\nformaline, default mkDir Error-->', this.uploadRootDir,'message:', ierr.message );
                this.emitException( 'pathexception', this.uploadRootDir, 'Default Dir Creation Error' + ierr.message );
                return;
            }
        }
        ...
        if( clength ){
            ...
            try{
                ...
                //error
                this.emitException( 'headersexception', isUpload , 'req.headers[content-length]: '+ parseIntError + ', length value:' + clength );
                return;
            }
            if( ! this.validatePostSize( bytes2Receive ) ){
                this.emitException( 'headersexception', isUpload, 'req.headers[content-length] exceeds max allowable: ' + bytes2Receive + ' > ' + this.uploadThreshold );
                return;
            }
           
        }else{
            //error
            this.emitException( 'headersexception', isUpload , 'req.headers[content-length] not defined:  Parse Length Error' );
            return;
        }
        ...
        if( ctype ){
            ...
            //error
            this.emitException( 'headersexception', isUpload, 'req.headers[content-type] is not defined: Parse Type Error' );
            return;
        }
    }else{
        //error
...
FPROTO.sendUrlEncodedResponse = function(){
...
FPROTO.sendMultipartResponse =  function(nbytes){                 
    this.endTime = new Date().getTime();
    this.endResults = {
