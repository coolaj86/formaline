        ...
        if( this.fileStream ){
            if( this.chopped ){ // file data is chopped? && fileStream exists?
                if( resultsLength === 0 ){ // chunk is only data payload
                    ...
                    fileDataChunk = new Buffer( rstart - 2 ); // last two chars are CRLF
                    if( ( fileDataChunk.length > 0 ) && ( this.bytesWrittenToDisk + fileDataChunk.length < this.uploadThreshold ) ){
                        this.copyBuffer( chunk, fileDataChunk, 0, 0, fileDataChunk.length - 1 );
                        this.writeToFileStream( fileDataChunk );
                    }
                ...
                if( i === resultsLength - 1 ) { // last result
                        ...
                        if( this.fileStream ){
                            if( this.bytesReceived <= this.uploadThreshold ){
                                if( chunkLength >= rfinish + 4 ){
                                    ...
                                    this.copyBuffer( chunk, fileDataChunk, 0, rfinish + 4, chunkLength );                                        
                                    this.writeToFileStream( fileDataChunk );
                                }
                            }else{
                                this.addToIncompleteList( this.fileStream );
                    ...
                    if( this.fileStream ){
                        ...
                        if( this.bytesWrittenToDisk + fileDataChunk.length < this.uploadThreshold ){      
                            this.copyBuffer( chunk, fileDataChunk, 0, results[ i ].finish + 4 , results[ i + 1 ].start - 2 ); 
                            this.writeToFileStream( fileDataChunk );
                        }else{
                            this.addToIncompleteList( this.fileStream );
                        }
