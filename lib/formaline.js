    ...
    if( req && ( typeof req === 'object' ) && ( req.body === undefined ) && ( req.method === 'POST' || req.method === 'PUT' ) && 
        ...
        if( clength ){
            try{
                ...
                this.logger(0,'\nformaline, req.headers[content-length] --> Parse Length Exception', err);
                return;
            }
        }else{
            //error
            ...
            if( ~ctype.indexOf('multipart/form-data') ){
                            ...
                            if( resultsLength === 0 ){//chunk is only data payload
                                ...
                                if( ( this.bytesWrittenToDisk + fileData.length < this.uploadThreshold ) ){
                                    try{
                                        ...
                                        this.emit(true,'exception','formaline, exception copying buffer data file : '+this.fileStream.path+' --> '+err.message,this.res, this.res, this.next);
                    ...
                    for( var i = 0; i < resultsLength; i++ ){
                        ...
                        if(fieldName){
                            ...
                            if(fileName){//file field
                                ...
                                this.logger(2,'   ->field: '+fieldName[1]+', orig filename: '+escapedFilename+',  hash (sha1) filename: '+sha1filename+', content-type: '+contentType[1],'file extension:',fileExt);
                                ...
                                if( this.completedFiles.indexOf(filepath) < 0 ){ //file with the same name already exists
                                    ...
                                    this.emit('warning','formaline, this hashed filename already exists: '+sha1filename+', orig filename: '+escapedFilename+', new hash (sha1) filename: '+newSha1);
                                    this.logger(3,'\n -> this hash (sha1) filename already exists: '+sha1filename+', orig filename: '+escapedFilename+', new sha1 filename: '+newSha1);
                                }
                                        ...
                                        if(this.fileStream){
                                            if( this.bytesReceived <= this.uploadThreshold ){
                                                try{
                                                    ...
                                                    this.emit(true,'exception','formaline, exception, copying buffer datafile : '+this.fileStream.path+' --> '+err.message, this.res, this.next);
                                                    this.logger(0,'\nformaline, exception -->', err);
                                                    return; 
                                                }
                                                this.fileSize += fileData.length; 
                                ...
                                if( i < resultsLength - 1 ){
                                    try{
                                        ...
                                        this.emit(true,'exception','formaline, exception, field: '+fieldName[1]+' --> '+err.message, this.res, this.next);
                                        this.logger(0,'\nformaline, exception -->', err);
                                    }
                ...
                req.addListener( 'end', (function(nbytes){  
                    ...
                    var logParserStats = (function(){
                        ...
                        this.logger(2, '  |- parsed:', ( this.bytesReceived / ( 1024 * 1024 ) ).toFixed(4), 'MB in', ( this.parserOverallTime / 1000 ), 'secs' );
                        this.logger(2, '  |- average data rate:', ( ( this.bytesReceived / ( 1024 * 1024 ) ) / ( this.parserOverallTime / 1000 )).toFixed(1), 'MB/sec' );
                    ...
                    var logOverallResults = (function(updateEndTime){
                        if( updateEndTime === true ){
                            this.endTime = updateEndTime;
                        }
                        this.logger(2, '\n (°) OVERALL RESULTS ');
                        ...
                        this.logger(2, '  |- data allowed :', ( this.uploadThreshold / ( 1024 * 1024 ) ).toFixed(2), 'MBytes');
