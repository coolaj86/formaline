...
fproto.emitEvent = function( type, obj, logLevel ){
    ...
    if( type.indexOf( 'exception' ) > - 1 ){
        ...
        if( ( etype === 'abort' ) || ( etype === 'timeout' ) ){
            ...
            /** /
            var groupResults = function( hash ){
                    var arr = [];
                    for ( var h in hash ){
                ...
                obj = { 
                    files : groupResults( this.receivedFilesCollection.hash ),
                    incomplete: groupResults( this.incompleteFilesCollection.hash ),
                    fields : this.receivedFieldsCollection.hash,
                    stats : {
            ...
            // TODO remove current incomplete fileStream and / or add to incompleteFilesCollection /incompleteFiles !
            
            this.logger( 2, '\n formaline, event: "' + 'loadend' + '" --> \n' , obj );
            this.emit( 'loadend', obj, this.res, this.next );
        /**/
        // TODO move to shared function
            var addToIncompleteList = ( function( file ){
            ...
            if( this.fileStream ) { addToIncompleteList( this.fileStream ); } // TODO!! if it is finished?
            this.sendResponseToMultipart( ( this.req.headers ) ? this.req.headers[ 'content-length' ] : 0 );
...
fproto.sendResponseToMultipart =  function( nbytes ){ 
    ...
    if( this.removeIncompleteFiles === false ){
        sendResponse( { 
            files : groupResultsByFieldName( this.receivedFilesCollection.hash ), 
            incomplete : groupResultsByFieldName( this.incompleteFilesCollection.hash ), 
            fields : this.receivedFieldsCollection.hash,
            stats : this.endResults
        } );
    }else{
