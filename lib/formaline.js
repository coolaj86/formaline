    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                // comment --> || ( this.incompleteFiles.indexOf( filepath ) > -1 ) ) for discarding ( nor writing to disk nor signaling event ) files that exceed the max upload threshold
                if( ( this.completedFiles.indexOf( filepath ) > -1 ) || ( this.incompleteFiles.indexOf( filepath ) > -1 ) ){ 
                    ...
                    jsonWarnFileExists.msg = 'this (sha1) file name already exists --> ' + path.basename( filepath ) + '( filename: ' + escapedFilename + ', fieldname: ' + fieldName[ 1 ] + ' )';
...
fproto.sendResponseToMultipart =  function( nbytes ){ 
    ...
    var logParserStats = ( function(){
        ...
        logOverallResults = ( function( updateEndTime ){
            ...
            this.logger( 1, '  |- partially written files :', this.incompleteFiles.length + '\n' );
