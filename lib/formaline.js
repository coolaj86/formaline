    ...
    formaline = function ( config ) {
        ...
        if( config && ( typeof config === 'object' ) ) {
            ...
            if (config.uploadRootDir) {
                config.uploadRootDir = path.normalize(config.uploadRootDir) + '/';
            }
...
fproto.parse = function ( req, res, next ) {
    ...
    var hs = req.headers,
        ...
        createUploadDir = function ( cback ) {
            ...
            fs.exists( me.uploadRootDir, function ( exists ) {
                if ( exists ) {
                    ...
                    fs.exists( me.uploadRootDir, mkSubDir );
                } else {
        ...
        createUploadDirSync = function ( cback ) {
            if ( fs.existsSync( me.uploadRootDir ) ) {
                me.logger( 3, 'formaline, upload root dir exists : "' + me.uploadRootDir + '"' );
                me.uploadRootDir = me.uploadRootDir + getUploadSubDirectoryName() + dirSeparator; 
            } else { 
                // uploadRootDir doesn't exist
            ...
            if ( ! fs.existsSync( me.uploadRootDir ) ) { // if subdirectory doesn't already exist, create it
                 try {
...
fproto.parseMultipartData = function ( chunk, cback ) {
    ...
    me.qkp = ( qkp ) ? ( qkp ) : new QuickParser( me.boundBuffer );
    qkp = me.qkp;
