...
var formaline = function ( config ){
    ...
    this.emitField = ( function( fieldname, fieldvalue ){
        this.logger( 3, '\nformaline, field -->  name:', fieldname, 'value:', fieldvalue );
        this.emit( 'field', fieldname, fieldvalue);
    } ).bind( this ),
        
    this.emitFileReceived = ( function( sha1name, origname, path, type, size, fieldname, datasha1sum ){
            this.logger( 2, '\n filereceived -->  sha1name: ' + sha1name + ', original name: ' +  origname + ', path: ' + path + ', type: ' + type + ', bytes: ' +  size + ', field: ' + fieldname + ', data sha1 sum: ' + datasha1sum + '\n' );
            this.emit( 'filereceived', sha1name, origname, path , type, size, fieldname, datasha1sum );
            
    } ).bind( this ),
     
    this.emitProgress = ( function( bytes, chunks, ratio ){
...
FPROTO.sendResponseToUrlEncoded = function(){
    ...
    for( var f in fields ){
        this.emitField( f, field[ f ] );
        //this.logger( 2, ' -> field:' + f + ', data: *' + field[ f ] +  '*' );
...
FPROTO.parseMultipartData = function( chunk ){
    ...
    var writeToFileStream = ( function( dataPayload, cfg ){  
        ...
        addToCompletedList = ( function( file ){
            ...
            //this.logger( 2, '\n filereceived -->  sha1name: ' + path.basename( file.path )+ ', original name: ' +  file.origname + ', path: ' +  path.dirname( file.path ) + ', type: ' + file.ctype + ', bytes: ' +  this.fileSize + ', field: ' + file.fieldname + ', data sha1 sum: ' + filedatasha1sum + '\n' );
            //this.emit( 'filereceived', path.basename( file.path ), file.origname, path.dirname( file.path ), file.ctype, this.fileSize, file.fieldname, filedatasha1sum );
        } ).bind( this ),
    ...
    for( var i = 0; i < resultsLength; i++ ){
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                ...
                this.logger( 2,' formaline, file field --> field name: ' + fieldName[ 1 ] + ', original filename: ' + escapedFilename + ', new (sha1sum) filename: ' + sha1filename + ', content-type: ' + fieldCtype );
                //TODO emitting also field event ?
                ...
                if( i === resultsLength - 1 ) { // last result
                    ...
                    if( this.fileStream ){
                        ...
                        closeFileStream( this.fileStream ); //TODO move upper ?
                        resetFileStream();
                    }
                }    
            }else{ // normal field
                if( i < resultsLength - 1 ){
                  ...
                  //TODO
                    ...
                    //this.logger( 2, ' -> field:', fieldName[ 1 ] + ', content-type: ' + fieldCtype , 'data: *', fileDataChunk.toString(), '*' );
                ...
                fs.unlink( currfile, (function( err, cfile, i, len ){
                    if( err ){
                        this.emitException( 'warning', isUpload, 'Warning: file unlink exception -> ' + cfile+', directory -> ' + this.uploadRootDir, 0 );
                         ...
                         this.logger( 2, '\n fileremoved -->  sha1name: ' + path.basename( cfile )+ ', original name: ' +  ifile.origname + ', path: ' + ifile.dir + ', type: ' +  ifile.ctype + ', bytes: ' + ifile.rbytes + ', field: ' + ifile.field  + ', no data sha1 sum ' + '\n' );
