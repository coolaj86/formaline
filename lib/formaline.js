...
FPROTO.parseMultipartData = function( chunk ){
    ...
    this.writeToFileStream = function( dataPayload, cfg ){  
        try{
            if( dataPayload ){
    ...
    this.generateHashFileName = function( fname ){
        ...
        if( this.fileStream ){
            if( this.chopped ){ // file data is chopped? && fileStream exists?
                if( resultsLength === 0 ){ // chunk is only data payload
                    ...
                    this.resetFileStream( this.fileStream );
                }
            }else{
        ...
        if( fieldName ){
            ...
            if( fileName ) { // file field
                var escapedFilename = fileName[ 1 ].replace( escapeChars, '' ),
                    sha1filename = this.generateHashFileName( escapedFilename ),
                ...
                if( this.completedFiles.indexOf( filepath ) > -1 ){ 
                    filepath = this.uploadRootDir + this.generateHashFileName( ( new Date().getTime() ) + '_' + escapedFilename  );
