...
var lookupTable = function(p){ 
    ...
    clrfclrf = new Buffer( [ 13, 10, 13, 10 ] ),
    
    crlfcrlfTable = new lookupTable( clrfclrf ), // 0x0d 0x0a 0x0d 0x0a
    
    clrfcrlfRawBuffer = crlfcrlfTable.getRawBuffer();
    
    crlfcrlfMatch = function ( t, l ) { //stop at first match of CRLFCRLF, obviously.. l is start index 
    
        var p = clrfclrf,// new Buffer([13,10,13,10]), //4 bytes CRLFCRLF
            m = p.length,
            n = t.length,
            s = l || 0,
            ok = 1,
            lkt = crlfcrlfTable, 
            lkb = clrfcrlfRawBuffer;
